...
function isLanguageDiverse(list) {
    //count different languages

    let javascript = list.filter(person => person.language === 'JavaScript').length
    let ruby = list.filter(person => person.language === 'Ruby').length
    let python = list.filter(person => person.language === 'Python').length

    // divide the lowest by the highest

    let ratio = Math.max(javascript, ruby, python) / Math.min(javascript, ruby, python)

    // return true if result is lower than 2
